...
DIST_COMMON = README $(am__configure_deps) $(srcdir)/Makefile.am \
	$(srcdir)/Makefile.in $(srcdir)/chasen-config.in \
	$(srcdir)/chasen.spec.in $(srcdir)/config.h.in \
	$(top_srcdir)/configure AUTHORS COPYING ChangeLog INSTALL NEWS \
	acconfig.h config.guess config.sub depcomp install-sh ltconfig \
...
chasen-config: $(top_builddir)/config.status $(srcdir)/chasen-config.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
chasen.spec: $(top_builddir)/config.status $(srcdir)/chasen.spec.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
